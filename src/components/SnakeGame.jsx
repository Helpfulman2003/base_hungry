...
import { payStartFee, payEndFee } from '../lib/contract.js';

// ── Fee status labels ──────────────────────────────────────────────
const FeeStatus = ({ status }) => {
  if (!status) return null;
  const map = {
    paying:  { icon: <Loader size={13} className="spinning" />, text: 'Paying fee…',    cls: 'fee-dot-warn' },
    ok:      { icon: <CheckCircle size={13} />,                  text: 'Fee paid ✓',     cls: 'fee-dot-ok'   },
    error:   { icon: <AlertCircle size={13} />,                  text: 'Fee skipped',    cls: 'fee-dot-none' },
  };
  const { icon, text, cls } = map[status] || {};
  return (
    <div className={`fee-status ${cls}`}>
      {icon} {text}
    </div>
  );
};
...
// ── Main component ─────────────────────────────────────────────────
const SnakeGame = ({ wallet, onBack }) => {
  const canvasRef    = useRef(null);
  const svgScreenRef = useRef(null);

  const { gameState, score, startGame, pauseGame, resetGame, setDirection } =
    useGameEngine(canvasRef, svgScreenRef);

  const [startFeeStatus, setStartFeeStatus] = useState(null); // null | 'paying' | 'ok' | 'error'
  const [endFeeStatus,   setEndFeeStatus]   = useState(null);
  const [txHash,         setTxHash]         = useState('');
  const [gameReady,      setGameReady]       = useState(false);

  // ── Pay start fee then launch game ───────────────────────────────
  const handleStart = useCallback(async () => {
    setStartFeeStatus(null);
    setEndFeeStatus(null);
    setTxHash('');
    setGameReady(false);

    if (wallet?.signer) {
      setStartFeeStatus('paying');
      try {
        const hash = await payStartFee(wallet.signer);
        setTxHash(hash);
        setStartFeeStatus('ok');
      } catch {
        setStartFeeStatus('error');
      }
    }
  ...
  // ── Pay end fee when game over ───────────────────────────────────
  useEffect(() => {
    if (gameState !== 'gameover') return;
    if (!wallet?.signer) return;

    const pay = async () => {
      setEndFeeStatus('paying');
      try {
        await payEndFee(wallet.signer);
        setEndFeeStatus('ok');
      } catch {
        setEndFeeStatus('error');
      }
    };
    pay();
  }, [gameState, wallet]);
  ...
  return (
    <div className="glass-panel game-wrapper">
      {/* HUD */}
      <div className="game-hud">
        ...
        <FeeStatus status={startFeeStatus || endFeeStatus} />
      </div>

      {/* Canvas */}
      <div className="game-canvas-container" style={{ width: '100%', maxWidth: 800 }}>
        <canvas ref={canvasRef} width={800} height={600} />

            {/* Dragon SVG — blue palette */}
            <svg
            ...
            {gameState === 'gameover' && (
              <div className="overlay" style={{ zIndex: 10 }}>
                <h2>Game Over!</h2>
                <p className="overlay-score">Final Score: <strong>{score}</strong></p>

                {endFeeStatus === 'paying' && (
                  <p className="overlay-status">
                    <Loader size={13} className="spinning" /> Processing end fee…
                  </p>
                )}
                {endFeeStatus === 'ok' && (
                  <p className="overlay-status" style={{ color: 'var(--success)' }}>
                    <CheckCircle size={13} /> End fee paid
                  </p>
                )}
                {txHash && (
                  <a
                ...
                <div className="overlay-actions">
                  <button className="btn" onClick={handleReplay} style={{ width: 'auto', minWidth: 120 }}>
                    Play Again
                  </button>
                  <button className="btn btn-secondary" onClick={onBack} style={{ width: 'auto', minWidth: 110 }}>
                    Menu
                  </button>
                </div>
              </div>
      ...
      <div className="mobile-controls">
        <div className="dpad">
          <button className="dpad-btn dpad-up"     onPointerDown={dpad(0, -1)}>▲</button>
          <button className="dpad-btn dpad-left"   onPointerDown={dpad(-1, 0)}>◀</button>
          <div    className="dpad-center" />
          <button className="dpad-btn dpad-right"  onPointerDown={dpad(1,  0)}>▶</button>
          <button className="dpad-btn dpad-down"   onPointerDown={dpad(0,  1)}>▼</button>
        </div>
      </div>
    </div>
  );
