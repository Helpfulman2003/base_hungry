 ...
 * @notice On-chain leaderboard for Raiku Snake game on Base Mainnet
 */
contract SnakeLeaderboard {
    ...
    struct PlayerStats {
        uint256 highScore;    // all-time best score
        uint256 totalGames;   // total games played
        uint256 totalScore;   // cumulative score across all games
        uint256 lastSubmit;   // anti-spam timestamp
        string  nickname;
    }
    ...
    // Events
    event ScoreSubmitted(address indexed player, string nickname, uint256 score, uint256 highScore);
    event LeaderboardUpdated(uint8 position, address player, uint256 highScore);
    ...
    function submitScore(
        ...
        if (score > ps.highScore) {
            ps.highScore = score;
        }

        emit ScoreSubmitted(msg.sender, nickname, score, ps.highScore);

        _updateLeaderboard(msg.sender, ps.highScore, ps.totalGames, ps.totalScore, nickname);
    }
    ...
    function _updateLeaderboard(
        ...
        for (uint8 i = 0; i < 10; i++) {
            ...
            if (insertAt == 10 && highScore > topPlayers[i].highScore) {
                insertAt = i;
            }
        }
        ...
        if (insertAt == 10) {
            ...
            if (oldIdx != 10) {
                topPlayers[oldIdx] = LeaderEntry(player, nickname, highScore, totalGames, totalScore, block.timestamp);
            }
            return;
        }
        ...
        if (oldIdx != 10) {
            if (insertAt < oldIdx) {
                ...
                topPlayers[insertAt] = LeaderEntry(player, nickname, highScore, totalGames, totalScore, block.timestamp);
            } else {
                topPlayers[oldIdx] = LeaderEntry(player, nickname, highScore, totalGames, totalScore, block.timestamp);
            }
        } else {
        ...
        emit LeaderboardUpdated(insertAt, player, highScore);
    }
}
